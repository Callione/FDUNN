...
class Conv2d(Module):
    ...
    def backward(self, output_grad):
        ...
        input_grad = np.pad(input_grad, ((0, 0), (0, 0), (self.padding, self.padding), (self.padding, self.padding)), mode='constant')
        ...
        for b in range(batch_size):
            for c_out in range(out_channels):
                for c_in in range(in_channels):
                    for i in range(output_height):
                        for j in range(output_width):
                            # print('kernel=',self.params['W'][c_out, c_in])
                            # print('patch=',output_grad[b, c_out, i, j])                   

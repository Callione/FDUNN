...
class CrossEntropyLoss(Loss):
    ...
    def forward(self, input, target):
        ...
        input = np.exp(input - np.max(input, axis=1, keepdims=True))
        input /= np.sum(input, axis=1, keepdims=True)
        ...
        # print('loss: ', loss)
        # print('target: ', target)
        loss = -np.sum(target * np.log(input+1e-15))
    ...
    def backward(self):
        ...
        #input_grad = -self.target[:, np.newaxis] / self.input
